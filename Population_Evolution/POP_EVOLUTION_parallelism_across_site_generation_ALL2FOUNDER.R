...
## This script will analyze the parallelism in GrENE-net using frequency change all compared to founder generation
...
##### calculate the per generation delta frequency change: compare to the founder generation
...
snp_parallelism <- snp_parallelism %>%
...
ecotype_parallelism <- ecotype_parallelism %>%
  left_join(.,site_name[,c(1,2)],by="site")
...
## for generation 0 to 2

site_gen2 <- meta$site[terminal_index$index2[index_gen_2]]
unique_site_gen2 <- unique(meta$site[terminal_index$index2[index_gen_2]])
delta_snp_gen2 <- apply(ld_p[,terminal_index$index2[index_gen_2]],2,function(x) x-ld_p0$V3)
delta_ecotype_gen2 <- apply(ecotype_p[,terminal_index$index2[index_gen_2]],2,function(x) x-ecotype_p0$V2)
...
delta_snp_gen3 <- apply(ld_p[,terminal_index$index3[index_gen_3]],2,function(x) x-ld_p0$V3)
delta_ecotype_gen3 <- apply(ecotype_p[,terminal_index$index3[index_gen_3]],2,function(x) x-ecotype_p0$V2)
...
ggplot(combined_summary,aes(x=generation,y=mean_r,colour = source)) +
  ...
  ylim(c(-0.08,0.6))+
  ylab("within-garden repeatibility (r)")+
...
ggsave(filename =  paste0("figs/",prefix,"per_generation_parallelism_summary_ALL2FOUNDER.pdf"),device = "pdf",width = 6,height = 4,units = "in")
...
combined_gen1 <- combined %>%
  ...
  filter(generation==1)
# combined_gen1$site
order_site <- combined_gen1$site[order(combined_gen1$mean[combined_gen1$source=="snp"],decreasing = T)]
...
p_snp <- ggplot(combined_snp,aes(x=site,y=mean,colour = generation,shape = generation))+
  ...
  scale_x_discrete(breaks = combined_snp$site,labels = paste0("#",combined_snp$site," ",combined_snp$name))+
  ylab("within-garden repeatibility (r)")+
  xlab("GrENE-net garden")+
  ggtitle("snp frequency change")+
  theme_classic()
p_snp
ggsave(plot = p_snp,filename =  paste0("figs/",prefix,"all_generations_parallelism_snp_ALL2FOUNDER.pdf"),device = "pdf",width = 6,height = 8,units = "in")
ggsave(plot = p_snp,filename =  paste0("figs/",prefix,"all_generations_parallelism_snp_ALL2FOUNDER.png"),device = "png",width = 6,height = 8,units = "in",dpi = 600)
...
p_ecotype <- ggplot(combined_ecotype,aes(x=site,y=mean,colour = generation,shape = generation))+
  ...
  geom_hline(yintercept = 0,linetype="dotted")+
  scale_color_manual(values =c("grey20","grey50","grey80"))+
  scale_x_discrete(breaks = combined_ecotype$site,labels = paste0("#",combined_ecotype$site," ",combined_ecotype$name))+
  ylab("within-garden repeatibility (r)")+
  xlab("GrENE-net garden")+
  ggtitle("founder frequency change")+
  theme_classic()
p_ecotype
ggsave(plot = p_ecotype,filename =  paste0("figs/",prefix,"all_generations_parallelism_ecotype_ALL2FOUNDER.pdf"),device = "pdf",width = 6,height = 8,units = "in")
ggsave(plot = p_ecotype,filename =  paste0("figs/",prefix,"all_generations_parallelism_ecotype_ALL2FOUNDER.png"),device = "png",width = 6,height = 8,units = "in",dpi = 600)



p_combined <- p_snp | p_ecotype
p_combined
ggsave(plot = p_combined,filename =  paste0("figs/",prefix,"all_generations_parallelism_snp_ecotype_ALL2FOUNDER.pdf"),device = "pdf",width = 16,height = 10,units = "in")
