...
## This script will use the PRS estimated ecotype origin to generate the stabilizing selection data frame which has the meta information of all ecotypes,
...
ecotype_climate <- read.delim("data-intermediate/CLIMATE_GWAS_bslmm_prs_ecotype_climate.txt",sep="\t")
colnames(ecotype_climate) <- c("ecotype",paste0("bio",1:19,"_ecotype_prs"))
...
write.table(mydata_site_merged,paste0("data-intermediate/",prefix,"stabilizing_selection_data_2024Jun18_collectionsite_BSLMM_PRS_allyears_pt1_pt0.txt"),append = F,quote = F,sep = "\t",row.names = F)
