...
for ( i in 13:19){
  ...
  p <- ggplot(df_bio,aes(x=diff,y=pt1-pt0))+
    ...
    ggtitle(paste0("R2_model: ",round(R2,4),"   R2_Vs:",round(R2_vs,4) ))+
    theme_classic()
  ggsave(p,filename =  paste0("figs/stabilizing_selection_per_bio/",prefix,"stabilizing_selection_generation_1_bio",i,".pdf"),device = "pdf",width = 5,height = 4,units = "in")
  ggsave(p,filename =  paste0("figs/stabilizing_selection_per_bio/",prefix,"stabilizing_selection_generation_1_bio",i,".png"),device = "png",width = 5,height = 4,dpi = 600)
