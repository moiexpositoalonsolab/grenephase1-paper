...
results_dir = sprintf('/carnegie/nobackup/scratch/tbellagio/gea_grene-net/leave_1_out_sv/split_number_%d', split_number)
...
env_test <- read.csv(sprintf('%s/environment_test.csv', results_dir), header = TRUE, colClasses = "numeric")
bio1 = env_test[, 'bio1']
bio1 <- as.matrix(bio1)
...
## get the allele freq 
allele_freq <- fread(delta_p_normed_file, select = train_samples_vector, sep = ',', header = TRUE, check.names = FALSE)
