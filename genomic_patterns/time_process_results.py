...
# This script processes the results from regression analysis of allele frequency changes
# across different sites. It reads JSON results files, converts them to CSV format,
# and identifies statistically significant results using a Bonferroni-corrected p-value threshold.
# The script also saves significant results to separate files.
...
# - results_persite/*.jsonl (Results from regression analysis)
# The script will write output to:
# - results_persite/site_{site}.csv (Processed results for each site)
# - results_persite/sign_site_{site}.csv (Significant results for each site)
...
# --- Identify and Save Significant Results ---
# Find all CSV result files
result_files = [i for i in os.listdir('results_persite') if 'csv' in i]

# Process each result file to identify and save significant results
for i in result_files:
    ...
    sign_site.to_csv(f'results_persite/sign_{i}')
