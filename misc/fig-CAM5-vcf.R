...
# Read VCF
vcf_file<-"data-external/arabidopsistranscriptomes/subset_CAM5.recode.vcf"
...
our_popmap<-data.frame( ind=colnames(vcf_subset@gt)[-1],
            ...
            dplyr::mutate(pop=round(bio1_new)) %>%
...
# Save
save_plot(
  paste0("figs/fig-CAM5-vcf-genemodel.pdf"),
  vcf_cam5,
...
# Define the chromosome and position
chrom <- "2"
pos <- "11533937"

# Subset the 'fix' slot for the specific position
specific_variant <- vcf_subset@fix[vcf_subset@fix[, "CHROM"] == chrom & vcf_subset@fix[, "POS"] == pos, ]

# View the variant details
print(specific_variant)


# Get the row index of the specific variant
row_index <- which(vcf@fix[, "CHROM"] == chrom & vcf_subset@fix[, "POS"] == pos)

# Extract genotype information for this variant
genotype_data <- vcf@gt[row_index, ]

# View the genotype information
print(genotype_data)
length(genotype_data)

alternativegenotypes<-
  names(genotype_data[genotype_data=="1/1"]) %>% unique()
alternativegenotypes<-
  data.frame(id=alternativegenotypes) %>% 
  na.omit %>% 
  tidyr::separate(id,c("id","fam"))

ara<-read.csv("~/natvar/data/Arabidopsis_thaliana_world_accessions_list.csv")

ara %>% 
  dplyr::filter(id %in% alternativegenotypes$id)
