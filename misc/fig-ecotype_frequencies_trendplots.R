...
# 12
ex<-ecofreq %>% 
  dplyr::filter(site==4, rep %in% 1:12)

fig_ecotype_mullerplot_allreps<-
  ggplot(ex, 
         aes(x = year, y = freq, fill = as.factor(id), group = as.factor(id))) + 
  geom_area()+
  scale_fill_manual(values=colorRampPalette(aracolors)(length(unique(ex$id))))+
  xlab("Years")+ylab("Ecotype frequency")+
  facet_wrap(~rep,ncol=3,nrow=4)+
  theme_minimal()+
  theme(legend.position = "none")

fig_ecotype_mullerplot_allreps

save_plot("figs/fig_ecotype_mullerplot_site4_repall.pdf",fig_ecotype_mullerplot_allreps,
          base_width = 10,base_height = 10 )
save_plot("figs/fig_ecotype_mullerplot_site4_repall.png",fig_ecotype_mullerplot_allreps,
          base_width = 10,base_height = 10)

# Three replicates
ex<-ecofreq %>% 
  dplyr::filter(site==25, rep %in% c(1,2,3))

fig_ecotype_mullerplot_severalreplicates<-
  ggplot(ex, 
         aes(x = year, y = freq, fill = as.factor(id), group = as.factor(id))) + 
  geom_area()+
  scale_fill_manual(values=colorRampPalette(aracolors)(length(unique(ex$id))))+
...
save_plot("figs/fig_ecotype_mullerplot_site32_rep1-3.pdf",fig_ecotype_mullerplot_severalreplicates,
          base_width = 10,base_height = 5 )
save_plot("figs/fig_ecotype_mullerplot_site32_rep1-3.png",fig_ecotype_mullerplot_severalreplicates,
          base_width = 10,base_height = 5 )
