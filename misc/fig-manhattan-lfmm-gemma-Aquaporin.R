...
lfmmraw <- read.csv("data-intermediate/gea/lfmm_last_gen/lfmm_bio18_k16_results.csv")
lfmmraw<-
  lfmmraw %>%
  mutate(snp=snp_id) %>%
  rename(P=p_value) %>%
...
path_gemma='data-intermediate/gea/climate_gwas/lmm_gemma/bio18/output/results_lmm.csv'
...
myfile=paste0("figs/fig-climate_gwas_lfmm_",biovariable,"_zoom-",
              paste0(mygene,"-",
                     myatgene),".png")
