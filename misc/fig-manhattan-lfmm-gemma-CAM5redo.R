...
regionplot(list(lfmmraw), 
           region=myregion, 
           build = annotations,
           title= paste(myatgene,mygene),
           annotate_with_vline = 1e-3, color=c("#41AB5D"),
           legend_labels = c("GrENE-net GEA"), show_gene_legend = F,
           gene_color="#646464")

dev.off()

annotations %>% 
  dplyr::filter(gene_id==myatname)
