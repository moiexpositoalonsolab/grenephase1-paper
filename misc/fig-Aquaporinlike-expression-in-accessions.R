...
genesubset<-d %>% dplyr::select(id,AT1G79780)
transcriptsubset<-d2 %>% data.frame() %>%  dplyr::select(id,AT1G79780.1)
...
mer<-merge(genesubset,worldclim_ecotypesdata, by.x='id',by.y="ecotypeid") %>% 
      mutate(mygene=AT1G79780)
mer2<-merge(transcriptsubset,worldclim_ecotypesdata, by.x='id',by.y="ecotypeid") %>% 
      mutate(mygene=AT1G79780.1)
################################################################################
# Plot cam5 and temp
ggplot(mer)+
  geom_point(aes(y=mygene, x=bio18))+
  stat_smooth(aes(y=mygene, x=bio18),method='glm')+
  scale_x_log10()
...
ggplot(mer2)+
  geom_point(aes(y=mygene, x=bio18))+
  stat_smooth(aes(y=mygene, x=bio18),method='glm', color= "#2166AC")+
  labs(x="Annual precipitaiton of accession origin (mm)", y=myatname)+
  scale_x_log10()
transcript1


save_plot(paste0("figs/fig_Aquaporinlike_expression.pdf"),transcript1,
          base_width = 5,base_height = 5 )
save_plot(paste0("figs/fig_Aquaporinlike_expression.png"),transcript1,
          base_width = 5,base_height = 5 )


sink("tables/test-Aquaporinlike-transcript-temperature-correlation.txt")
...
cor.test(mer2$mygene, log10(mer$bio18), method='p')
cor.test(mer2$mygene, log10(mer$bio18), method='s')
cor.test(mer2$mygene, log10(mer$bio18), method='k')
