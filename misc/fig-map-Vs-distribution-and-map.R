...
## Intermediate data
# Vsresults<-read.table("data-intermediate/1001g_grenet_genomewide_partition.txt",header = T) %>% 
...
load("data-intermediate/vssitesyearsperreps.rda")
head(vssites)

# load("data-intermediate/step1_ecotype_Vs.RData")
# Vseco=alldf
# load("data-intermediate/step2_site_tau.RData")
# Vssite=alldf
...
##### Only terminal year '
tmp2= merge(vssitesterminal, by.x="site", worldclim_sitesdata, by.y="site") %>% 
  dplyr::filter(longitude > -15)

vssitemap<-
  basemap+
  # ggplot()+
    geom_point(data=tmp2, aes(x=longitude, y=latitude, color=Vssite))+
...
vssitemap
save_plot("figs/fig-Vs-persite-map-terminal.pdf",vssitemap, 
          base_height = 4,base_width = 6)
save_plot("figs/fig-Vs-persite-map-terminal.png",vssitemap, 
          base_height = 4,base_width = 6)

##### All years
# Merge with climate and latlon
tmp2= merge(vssitesyears, by.x="site", worldclim_sitesdata, by.y="site") %>%
      # dplyr::filter(year==2) %>%
      dplyr::filter(longitude > -15)
# basemap+
vssitemap<-
  basemap+
  # ggplot()+
  geom_jitter(data=tmp2, aes(x=longitude, y=latitude, color=Vssite),height = 0.5, width = 0.5)+
  scale_color_gradientn(colors=c( rev(brewer.pal(5,"Reds")[1:5]),(brewer.pal(5,"Greens")[1:5]) ))
vssitemap
# save plot
save_plot("figs/fig-Vs-persite-map-years-jitter.pdf",vssitemap, 
          base_height = 4,base_width = 6)
save_plot("figs/fig-Vs-persite-map-years-jitter.png",vssitemap, 
          base_height = 4,base_width = 6)

##### All years and all replicates
# Merge with climate and latlon
tmp2= merge(vssitesyearsperreps, by.x="site", worldclim_sitesdata, by.y="site") %>%
  dplyr::filter(longitude > -15)
# Plot 
vssitemap<-
# ggplot()+
  ...
  scale_color_gradientn(colors=c( rev(brewer.pal(5,"Reds")[1:5]),(brewer.pal(5,"Greens")[1:5]) ))
# save plot
save_plot("figs/fig-Vs-persite-map-years-replicates-jitter.pdf",vssitemap, 
          base_height = 4,base_width = 6)
save_plot("figs/fig-Vs-persite-map-years-replicates-jitter.png",vssitemap,
          base_height = 4,base_width = 6)

###############################
# Create a model by climate

totrain= merge(vssitesyears, by.x="site", worldclim_sitesdata, by.y="site") %>%
          dplyr::filter(longitude > -15) %>% 
          dplyr::mutate(Vssite=ifelse(Vssite>0,0,Vssite))

totrain %>% 
  dplyr::select(Vssite, 
                # latitude, longitude, 
                starts_with("bio")) %>% 
  # lm(data=.,Vssite ~.) -> vsmod
  randomForest(data=.,Vssite ~.) -> vsmod
vsmod 
vsmod %>% summary

# What about predicting Vs using climate
# source("function-load-climate-maps.R")
vspred<-predict(now,vsmod)
plot(vspred)
...
fig_wmax_bfio1<-
  ...
  theme_minimal()
fig_wmax_bio1
save_plot("figs/fig-wmax-ecotype-mcmcglmm-and-bio1.pdf",fig_wmax_bfio1, base_height = 5,base_width = 6)
save_plot("figs/fig-wmax-ecotype-mcmcglmm-and-bio1.png",fig_wmax_bfio1, base_height = 5,base_width = 6)

fig_wmax_bio12<-
  ggplot(dfeco,
         aes(y=Wmax, x=bio12, color=1/Vs))+
  geom_point()+
  scale_color_gradientn("1/Vs",colors=c( rev(brewer.pal(5,"Reds")[1:5]),(brewer.pal(5,"Greens")[1:5]) ))+
  stat_smooth(formula = y~poly(x,2), method="glm", col="grey")+
  stat_smooth(formula = y~x, method="glm", col="grey")+
  ...
  theme_minimal()
fig_wmax_bio12
save_plot("figs/fig-wmax-ecotype-mcmcglmm-and-bio12.pdf",fig_wmax_bio12, base_height = 5,base_width = 6)
save_plot("figs/fig-wmax-ecotype-mcmcglmm-and-bio12.png",fig_wmax_bio12, base_height = 5,base_width = 6)
...
dfeco %>% 
  mutate(Vsinv=1/Vs) %>% 
  lm(data=.,formula= bio1 ~ Wmax * Vsinv) %>% 
  summary
...
fig_vs_suitability_wmaxsize<-
  ggplot(dfeco)+
  geom_point(aes(y=Wmax, x=suitability, color=1/Vs, size=Wmax))+
  scale_color_gradientn("1/Vs",colors=c( rev(brewer.pal(5,"Reds")[1:5]),(brewer.pal(5,"Greens")[1:5]) ))+
  stat_smooth(aes(y=Wmax, x=suitability), formula = y~poly(x,2), method="glm", col="grey")+
  stat_smooth(aes(y=Wmax, x=suitability), formula = y~x, method="glm", col="grey")+
  geom_rug(aes(x=suitability),col=  "black" )+
  # geom_rug(aes(x=bio1),col= color_palette() )+
  theme_minimal()
fig_vs_suitability_wmaxsize
