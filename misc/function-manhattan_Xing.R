...
manhattan_new <- function (x, chr = "CHR", bp = "BP", p = "P", snp = "SNP", col = c("gray32", 
                                                                                    "gray72"), chrlabs = NULL, suggestiveline = -log10(1e-05), 
                           ...
                           annotatePval = NULL, annotateTop = TRUE, ...) 
{
  ...
  if (!is.null(highlight)) {
    ...
    with(d.highlight, points(pos, logp, col = "#B30000", pch = 20, 
                             ...))
  }
...
## data$CHR <- Chromosome number
## data$BP <- position 
## data$SNP <- SNP name (usually I use CHR_BP)
## data$P <- pvalue
## sig_snps <- SNP names that you want to highlight

## Example code

manhattan_new(data,suggestiveline = F,genomewideline = F,highlight = sig_snps,col = c("grey32","grey72"),cex=0.6,ylim=c(0,20+1),
