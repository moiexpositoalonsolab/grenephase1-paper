...
"""
Population Dynamics Analysis - Population Size Estimates
This script analyzes estimated population sizes across three generations at different experimental sites.
It correlates population sizes with environmental variables (bio1 - annual mean temperature) and generates
visualizations showing temporal trends in population dynamics.

Input:
- pop_size_estimations.csv: Population size estimates
- bioclimvars_experimental_sites_era5.csv: Climate data
- generation_1_parallelism.txt: Parallelism data

Output:
- Various plots showing population dynamics across generations
...
# Load population size estimates
...
# Create site_plot identifier and pivot data
pop_size['site_plot'] = pop_size['site'].astype(str) + '_' + pop_size['plot'].astype(str)
pop_size = pop_size[['totalplantnumber_complete','site_plot', 'generation']].pivot_table(
    index='site_plot', 
    columns='generation', 
    values='totalplantnumber_complete'
).reset_index()
...
# Save final plot
plt.tight_layout()
plt.savefig('parallelism_ecotypes_cuadratic_term_pop_estimates.png')
plt.savefig('parallelism_ecotypes_cuadratic_term_pop_estimates.pdf')
